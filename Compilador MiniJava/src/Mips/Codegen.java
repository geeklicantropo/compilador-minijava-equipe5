...
public class Codegen {
    ...
    List<Temp> L(Temp h, List<Temp> t) {return new List<Temp>(h,t);} 
    ...
    void munchStm(Stm s) { 
		if (s instanceof Tree.MOVE) munchMove(((Tree.MOVE)s).dst, ((Tree.MOVE)s).src); 
	...
	void munchMove(MEM dst, Exp src) { 
		...
		if (dst.exp instanceof BINOP && ((BINOP)dst.exp).binop==BINOP.PLUS 
		    && ((BINOP)dst.exp).right instanceof CONST) {
			 emit(new OPER("STORE M['s0+" + ((CONST)((BINOP)dst.exp).right).value + "] <- 's1",
				 null, L(munchExp(((BINOP)dst.exp).left), 
					L(munchExp(src), null))));
		} 
		...
		// MOVE(MEM(e1), MEM(e2)) 
			...
			emit(new OPER("MOVEM M['s0] <- M['s1]", null,
		...
		// MOVE(MEM(e1), e2) 
			...
			emit(new OPER("STORE M['s0] <- 's1", null,
    ...
    private void munchJump(JUMP s) {
    	emit(new Assem.OPER("JUMP to "+s.targets.head.print(), null, L(new Temp(),null), s.targets));      
    }
    ...
    private void munchCJump(CJUMP s) {
        ...
        switch(s.relop) {
        case CJUMP.GE:
        	emit(new Assem.OPER("SUB `d0 <- `s0 - `s1", L(r, null), L(left, L(right,null)), null));
            emit(new Assem.OPER(true,"BRANCHGE if `s0 >= 0 goto "+lTrue+"", null, L(r, null), new List<Label>(lTrue, null)));
            emit(new Assem.OPER("JUMP to "+lFalse+"", null, L(t1,null), new List<Label>(lFalse, null)));
            break;
        case CJUMP.LT:
            emit(new Assem.OPER("SUB `d0 <- `s0 - `s1", L(r, null), L(left, L(right,null)), null));
            emit(new Assem.OPER(true,"BRANCHLT if `s0 < 0 goto "+lTrue+"", null, L(r, null), new List<Label>(lTrue, null)));
            emit(new Assem.OPER("JUMP to "+lFalse+"", null, L(t1,null), new List<Label>(lFalse, null)));
            break;
        case CJUMP.EQ:
            emit(new Assem.OPER("SUB `d0 <- `s0 - `s1", L(r, null), L(left, L(right,null)), null));
            emit(new Assem.OPER(true,"BRANCHEQ if `s0 == 0 goto "+lTrue+"", null, L(r, null), new List<Label>(lTrue, null)));
            emit(new Assem.OPER("JUMP to "+lFalse+"", null, L(t1,null), new List<Label>(lFalse, null)));
            break;
        case CJUMP.NE:
            emit(new Assem.OPER("SUB `d0 <- `s0 - `s1", L(r, null), L(left, L(right,null)), null));
            emit(new Assem.OPER(true,"BRANCHNE if `s0 != 0 goto "+lTrue+"", null, L(r, null), new List<Label>(lTrue, null)));
            emit(new Assem.OPER("JUMP goto "+lFalse+"", null, L(t1,null), new List<Label>(lFalse, null)));
        }
    }

    public Temp munchExp(Tree.Exp e) {
    ...
    public Temp munchCall(Tree.CALL e){
    	Temp r = munchExp(e.func);
    	List<Temp> l = munchArgs(0,e.args);
    	emit(new OPER("CALL 's0",null,L(r,l)));
    	return r;
    }
    
    public List<Temp> munchArgs(int iter, List<Exp> args) {
    	if (args == null) return null;
    	Temp r = munchExp(args.head);
    	List<Temp> l = new List<Temp>(r, munchArgs(iter+1,(List<Exp>)args.tail));
    	if(iter==0) {
    		List<Temp> l2=null;
    		// reverte a lista
    		for(;l!=null;l =(List<Temp>) l.tail){
    			l2 = new List<Temp>(l.head,l2);
    		}
    		return l2;
    	}
    	return l;
    ...
    private Temp munchMem(MEM e) {
        ...
        if(e.exp instanceof BINOP && ((BINOP)e.exp).binop == BINOP.PLUS && 
        	((BINOP)e.exp).right instanceof CONST ) {
                ...
                emit(new OPER("LOAD 'd0 <- M['s0 + "+((CONST)((BINOP)e.exp).right).value+ "]", 
                	L(r,null), L(munchExp(((BINOP)e.exp).left), null)));
                return r;
        }
        ...
        // MEM(BINOP(PLUS, CONST(i), e1))
        	...
        	((BINOP)e.exp).left instanceof CONST) {
                ...
                emit(new OPER("LOAD 'd0 <- M['s0 + "+((CONST)((BINOP)e.exp).left).value+ "]", 
                	L(r,null), L(munchExp(((BINOP)e.exp).right), null)));
                return r;
        }
        ...
        // MEM(e1)
        	...
        	Temp r = new Temp();
            emit(new OPER("LOAD 'd0 <- M['s0+0]", L(r,null),L(munchExp(e.exp),null)));
            return r;
        }        
    }
